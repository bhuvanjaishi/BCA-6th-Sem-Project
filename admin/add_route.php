...
if (isset($_POST['add'])) {
    ...
    // Corrected SQL query
    $sql = "INSERT INTO routes (route_id, origin, destination, distance,  duration,  status)
            VALUES ('$route_id', '$origin', '$destination', '$distance', '$duration',  '$status')";

    if ($conn->query($sql) === TRUE) {
        echo "<script>
                alert('New Route Add Successful');
...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Form */
        #add-route-form .form-group {
            margin-bottom: 18px;
        }

        #add-route-form label {
            display: block;
            margin-bottom: 6px;
            font-weight: bold;
            color: #333;
...
<div id="add-new-route">
    <div id="add-route-container">
        <div id="top-bar">
            <a href="manage_routes.php" id="back-link">← Back to Manage Routes</a>
            
            <h1 id="form-title"></i> Add New Route</h1>
        </div>

        <form action="" method="POST" id="add-route-form">
            ...
            <div class="form-group">
                <button type="submit" id="submit-btn" name="add">Add Route</button>
            </div>
        </form>
    </div>
</div>
